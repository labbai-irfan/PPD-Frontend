...
export function DesktopHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 hidden border-b border-border bg-background/90 backdrop-blur-lg md:block">
      <div className="mx-auto flex h-(--header-h) max-w-6xl items-center gap-6 px-8">
        <Link to={ROUTES.home} className="flex items-center gap-2.5" aria-label={`${APP_NAME} home`}>
          ...
          <span className="leading-tight">
            ...
            <span className="block text-[10px] font-medium text-muted-foreground">{APP_TAGLINE}</span>
          </span>
        </Link>
        ...
        <button
          ...
          className="flex h-11 max-w-md flex-1 items-center gap-2.5 rounded-full bg-card px-4 text-left shadow-pill cursor-pointer"
        >
        ...
        <div className="ml-auto flex items-center gap-3">
          <Link
            to={ROUTES.wishlist}
            aria-label="Wishlist"
            className="flex size-10 items-center justify-center rounded-full bg-card shadow-pill transition-transform hover:scale-105"
          >
