  ...
  return (
    <form onSubmit={handleSubmit(onSave)} className="space-y-4" noValidate>
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Input label="Mobile number" inputMode="numeric" placeholder="10-digit number" maxLength={10} error={errors.phone?.message} {...register('phone')} />
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Input label="Pincode" inputMode="numeric" placeholder="560001" maxLength={6} error={errors.pincode?.message} {...register('pincode')} />
        ...
        <div className="flex gap-2">
          {(['home', 'work', 'other'] as const).map((type) => (
            <button
              ...
              className={cn(
                'h-9 rounded-lg border px-4 text-sm font-medium capitalize transition-colors cursor-pointer',
                selectedType === type
