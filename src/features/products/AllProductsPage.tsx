  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between pt-2.5">
        <h1 className="text-[19px] font-bold text-foreground">All Products</h1>
        {chips.length > 0 && (
        ...
        <div className="no-scrollbar flex flex-1 gap-2 overflow-x-auto">
          {chips.map((chip) => (
            <button
              ...
              type="button"
              onClick={chip.remove}
              className="flex shrink-0 items-center gap-1 rounded-full bg-card px-3 py-[7px] text-[12.5px] font-medium text-ink-soft shadow-soft cursor-pointer dark:text-foreground"
            >
              <Icon name="cancel" size={16} fill className="text-accent" />
              {chip.label}
            </button>
          ))}
          {chips.length === 0 && <SortInline value={sort} onChange={(v) => update((p) => (v === 'relevance' ? p.delete('sort') : p.set('sort', v)))} />}
        </div>
        <CircleIconButton icon="filter_alt" iconSize={20} size={40} tone="solid" label="Filters" onClick={() => setFiltersOpen(true)} />
      </div>
          ...
          {categories?.map((c) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'rounded-full px-4 py-2 text-[12.5px] font-medium shadow-soft cursor-pointer',
