...
export default function AdminBannersPage() {
  ...
  const renderBannerCard = (banner: Banner, wide = false) => {
    const previewImage = banner.image || banner.items?.[0]?.image
    const previewImageMobile = banner.imageMobile || banner.items?.[0]?.imageMobile

    return (
      <Card key={banner.id} className="group overflow-hidden flex flex-col bg-white">
        <div className={`relative w-full overflow-hidden bg-slate-50 border-b border-slate-100 ${wide ? 'aspect-[3/1]' : 'aspect-[396/189]'}`}>
          {previewImage ? (
            <img
            ...
            <img
              src={mediaUrl(previewImageMobile)}
              alt={banner.title}
          ...
          {/* Floating Mobile Preview Overlay if both desktop and mobile exist */}
          {previewImage && previewImageMobile && (
            <div className="absolute bottom-2.5 right-2.5 w-11 h-20 border-2 border-white rounded-lg shadow-lg overflow-hidden bg-slate-950 z-20 transition-transform group-hover:scale-[1.04]">
              <img
                src={mediaUrl(previewImageMobile)}
                alt="Mobile preview"
          ...
          <span
            className={`absolute left-2.5 top-2.5 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide shadow-xs z-10 ${
              banner.isActive ? 'bg-emerald-500 text-white' : 'bg-slate-700/80 text-white'
            }`}
          >
            {banner.isActive ? 'Live' : 'Hidden'}
          </span>

          {/* Type Badge (Static/Carousel) */}
          <span className="absolute left-2.5 bottom-2.5 rounded-md bg-black/60 px-2 py-0.5 text-[9px] font-bold text-white uppercase tracking-wider backdrop-blur-xs z-10">
            {banner.type === 'carousel' ? '🎠 Carousel' : '🖼 Static'}
          </span>

          <span className="absolute right-2.5 top-2.5 rounded-full bg-black/50 px-2.5 py-1 text-[10px] font-semibold text-white backdrop-blur z-10">
            #{banner.sortOrder}
          </span>
        </div>
        <div className="flex flex-1 flex-col p-3.5 md:p-4">
          ...
          {banner.type === 'carousel' && (
            <p className="text-[11px] text-orange-600 font-bold mt-1.5 flex items-center gap-1">
              <span>🎠</span> {banner.items?.length || 0} Slides included
            </p>
          )}
          {banner.href && (
          ...
          <div className="mt-3 flex items-center gap-2 border-t border-border pt-3">
            <Button onClick={() => handleToggleActive(banner)} variant="outline" size="sm" className="flex-1 gap-1.5 cursor-pointer">
              {banner.isActive ? <EyeOff className="size-3.5" /> : <Eye className="size-3.5" />}
              {banner.isActive ? 'Hide' : 'Show'}
            </Button>
            <Button onClick={() => handleEdit(banner)} size="sm" className="flex-1 cursor-pointer">
              Edit
            </Button>
            <Button
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <Card className="w-full max-w-2xl space-y-4 p-6 max-h-[90vh] overflow-y-auto flex flex-col rounded-2xl border-none shadow-2xl bg-white">
            ...
            <div className="space-y-4 overflow-y-auto pr-1 flex-1">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Banner Type</label>
                <select
                  value={form.type}
              ...
              {form.type === 'static' && (
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Placement</label>
                  <select
              ...
              {form.type === 'static' ? (
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-semibold text-foreground mb-2">Banner Image (Desktop)</label>
                      <div className="flex gap-2">
                        <input
                          type="file"
                          accept="image/*"
                          onChange={(e) => e.target.files?.[0] && handleImageUpload(e.target.files[0], (url) => setForm({ ...form, image: url }))}
                          disabled={uploading}
                          className="flex-1 px-3 py-2 border rounded-lg bg-background text-foreground cursor-pointer text-xs"
                        />
                      </div>
                      {form.image && (
                        <div className="mt-2 relative w-full h-32 bg-muted rounded-lg overflow-hidden">
                          <img src={mediaUrl(form.image)} alt="Desktop Preview" className="w-full h-full object-cover" />
                        </div>
                      )}
                    </div>

                    <div>
                      <label className="block text-sm font-semibold text-foreground mb-2">Banner Image (Mobile - Optional)</label>
                      <div className="flex gap-2">
                        <input
                          type="file"
                          accept="image/*"
                          onChange={(e) => e.target.files?.[0] && handleImageUpload(e.target.files[0], (url) => setForm({ ...form, imageMobile: url }))}
                          disabled={uploading}
                          className="flex-1 px-3 py-2 border rounded-lg bg-background text-foreground cursor-pointer text-xs"
                        />
                      </div>
                      {form.imageMobile && (
                        <div className="mt-2 relative w-full h-32 bg-muted rounded-lg overflow-hidden">
                          <img src={mediaUrl(form.imageMobile)} alt="Mobile Preview" className="w-full h-full object-cover" />
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="space-y-3 border-t pt-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Button
                      onClick={() => {
                        const newItem: BannerItem = { title: '', subtitle: '', cta: 'Shop Now', href: '/', image: '', imageMobile: '' }
                  ...
                  {form.items?.map((item, idx) => (
                    <Card key={`item-${idx}`} className="p-4 border border-slate-100 shadow-3xs space-y-3 rounded-xl bg-slate-50/50">
                      <div className="flex items-center justify-between">
                        <h4 className="font-bold text-xs uppercase tracking-wider text-slate-500">Item {idx + 1}</h4>
                        <button
                          onClick={() => {
                            setForm({
                              ...form,
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-semibold text-foreground mb-1">Desktop Image</label>
                          <input
                            type="file"
                            accept="image/*"
                            onChange={(e) => {
                              if (e.target.files?.[0]) {
                                void handleImageUpload(e.target.files[0], (url) => {
                                  const updated = [...(form.items || [])]
                                  updated[idx] = { ...item, image: url }
                                  setForm({ ...form, items: updated })
                                })
                              }
                            }}
                            disabled={uploading}
                            className="w-full px-2 py-1 border rounded text-xs bg-background text-foreground cursor-pointer"
                          />
                          {item.image && (
                            <div className="mt-1 relative w-full h-16 bg-muted rounded overflow-hidden">
                              <img src={mediaUrl(item.image)} alt="Desktop Item Preview" className="w-full h-full object-cover" />
                            </div>
                          )}
                        </div>

                        <div>
                          <label className="block text-xs font-semibold text-foreground mb-1">Mobile Image (Optional)</label>
                          <input
                            type="file"
                            accept="image/*"
                            onChange={(e) => {
                              if (e.target.files?.[0]) {
                                void handleImageUpload(e.target.files[0], (url) => {
                                  const updated = [...(form.items || [])]
                                  updated[idx] = { ...item, imageMobile: url }
                                  setForm({ ...form, items: updated })
                                })
                              }
                            }}
                          ...
                          {item.imageMobile && (
                            <div className="mt-1 relative w-full h-16 bg-muted rounded overflow-hidden">
                              <img src={mediaUrl(item.imageMobile)} alt="Mobile Item Preview" className="w-full h-full object-cover" />
                            </div>
                          )}
                        </div>
                      </div>
            ...
            <div className="flex gap-3 pt-3 border-t justify-end">
              <Button
                ...
                variant="outline"
                className="h-10 px-5 text-gray-500 cursor-pointer"
              >
                Cancel
              </Button>
              <Button
